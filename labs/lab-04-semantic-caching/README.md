...
- **Azure Cache for Redis Enterprise** with the **RediSearch** module (required for vector similarity search)
- These are deployed automatically by `main.bicep` with `enableEmbeddingsBackend=true` and `enableEnterpriseRedis=true`

> **Note:** Redis Enterprise requires Marketplace to be enabled on your Azure subscription. If Marketplace purchases are disabled by policy, ask your tenant admin to enable it. Basic/Standard/Premium Redis tiers do **not** support the RediSearch module needed for semantic caching.
...
az apim backend create `
  ...
  --protocol "http" `
  --url "${AIS_ENDPOINT}openai/deployments/text-embedding-3-small/embeddings"
```
